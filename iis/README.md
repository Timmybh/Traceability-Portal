...
`SQLQUERY` phục vụ tab Truy suất RFID từ các bảng tracking. `SQLQUERY_NEW` phục vụ tab Truy suất RFID mới qua endpoint `/api/traceability/new`: RFID bắt đầu từ `CUTTING_TemBarcode_TachCay_RFID_Mapping`, nối `CUTTING_TemBarcode_TachCay.Code` để lấy thông tin. Tab mới không sử dụng bảng `Tracking_RFID_*`; Bàn cắt đang để trống cho đến khi xác định được luồng nguồn chính xác. Phiếu cấp BTP được nối theo mã hàng, PO và size để lấy xí nghiệp, chuyền và màu. LOT chính cùng LOT phối đều được xác định trong `CUTTING_PhieuCapBTP_BarcodeChiTiet` theo đúng `SoPhieuCapBTP`, PO và `IdCapBTPCT`; LOT phối được tách từ các chuỗi LOT còn lại sau khi loại LOT chính. `CUTTING_TemBarcode_TachCay.Lot` chỉ là dự phòng khi phiếu cấp không có LOT chính. Ngày may lấy từ `ThoiGianMap` và chỉ hiển thị ngày. Item, Art và Bàn cắt đang chờ xác định nguồn đúng. `SQLQUERY_NEW` chưa lấy timeline công đoạn và tạm trả `TimelineJson` rỗng cho đến khi xác định được các bảng nguồn mới.
