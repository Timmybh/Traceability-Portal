...
test("SQLQUERY uses tracking tables and SQLQUERY_NEW uses the cutting mapping chain", async () => {
  ...
  assert.match(sqlQueryNew, /STRING_SPLIT\(CAST\(d\.Lot AS nvarchar\(max\)\), N';'\)/);
  assert.match(sqlQueryNew, /d\.IdCapBTPCT\s*=\s*cap\.IdCapBTPCT/);
  assert.match(sqlQueryNew, /mainFabric\.LotVaiChinh/);
  assert.match(sqlQueryNew, /LTRIM\(RTRIM\(lotToken\.value\)\)\s*<>\s*LTRIM\(RTRIM\(mainFabric\.LotVaiChinh\)\)/);
  ...
  assert.match(sqlQueryNew, /CAST\(NULL AS nvarchar\(50\)\) AS BanCat/);
